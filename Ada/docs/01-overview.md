...
本教程共 **27 章**：01 章为全景与工具链（不带示例），02–26 章每章对应一个可编译、可运行的完整示例（**章号 = 示例编号**），27 章为附录。其中 08/09/13/17/19/21/22/26 八章是对照三本国内老教材（何诚《ADA 语言程序设计教程》、刘炳文《程序设计语言 Ada》、张丽芬《Ada 程序设计导论》，均 Ada 83 时代）深挖扩充的"进阶篇"，案例全部现代化重写并实测。推荐的学习路径：
...
本教程首轮验证在 **Windows（MSYS2 UCRT64，GNAT 16.1.0）** 上完成；2026-09-21 在 **Linux（GNAT 16.2.1）**、2026-09-22 在 **Windows（GNAT 16.2.0，章号对齐重构后）** 分别全量复验；2026-09-25 老教材扩充（19→27 章）后在 **Windows（GNAT 16.2.0）** 复验 25 个示例编译运行全部通过（平台差异与记录见 [27 章附录](27-appendix.md)）。
