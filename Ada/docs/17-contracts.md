...
> 示例：[`examples/ch17_contracts.adb`](../examples/ch17_contracts.adb)
...
```
failed precondition from ch17_contracts.adb:27
```
...
exception
   when E : Assertion_Error =>
      ...
      --  输出：failed precondition from ch17_contracts.adb:27
end;
```
...
完整代码见 [examples/ch17_contracts.adb](../examples/ch17_contracts.adb)，演示以下场景：
...
gnatmake -gnata -o examples\ch17_contracts.exe examples\ch17_contracts.adb
.\examples\ch17_contracts.exe
```

```bash
# Linux / macOS
gnatmake -gnata -o ch17_contracts examples/ch17_contracts.adb
./ch17_contracts
```
...
[7] 故意违反契约，演示异常捕获：
    7a. 违反 Pre (Y /= 0) 被捕获: failed precondition from ch17_contracts.adb:27
    7b. 违反 Dynamic_Predicate 被捕获: Dynamic_Predicate failed at ch17_contracts.adb:218
    7c. 违反 Pre (S.Top > 0) 被捕获: failed precondition from ch17_contracts.adb:113
...
---
上一章：[16 受保护对象](16-protected-objects.md) ｜ 下一章：[18 SPARK](18-spark.md) ｜ 返回：[README](../README.md)
