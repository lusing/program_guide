...
为了避免“先看控件再拼代码”的碎片化学习，这份指南按下面的逻辑组织：

1. **先统一概念地基**：WinUI 3 / XAML / WinRT / C++/WinRT 分别是什么、各自负责什么（第 2 章）
2. **再进入工程骨架**：应用结构、程序流程、事件/命令/状态流（第 3～7 章）
3. **再看界面能力**：控件、布局、绑定、异步更新（第 8～11 章）
4. **再做真实页面闭环**：Page / ViewModel / Model / Service 协作（第 12 章）
5. **再做应用级能力**：主题资源、Assets、Manifest、MSIX 打包交付（第 13～15 章）
6. **最后做质量闭环**：常见错误、学习路径、编译验证、示例索引（第 16～20 章）

你可以把它理解成一句话：

> 从“概念正确”到“结构正确”，再到“交付正确”。
...
### 2.6.1 `winrt::` 不是 STL，也不是自定义命名空间随便起的
...
### 2.6.3 `winrt::Windows::Foundation::IAsyncAction`：异步接口
...
### 2.6.4 `winrt::com_ptr` / `winrt::implements` / `winrt::make`：对象创建和接口实现
...
### 2.6.5 `winrt::event` / `event_token`：事件系统
...
#### 2.10.2 之后，Windows 平台开始要求更强的本机能力和更统一的应用模型
...
#### 2.10.3 所以，回到 C++，不是“否定 .NET”，而是“把 XAML 放回到原生 Windows 平台中”
...
#### 2.10.4 听起来像“又退回到了原生 C++”，但本质上是重新统一了平台架构
...
#### 2.11.3 实际上，它实现了，但不是以原来那种想象方式实现
...
- 与安装包相关的资源目录（如 `Assets/`）和 `Package.appxmanifest` 放在应用级结构中，详见第 14 章
- `Pages / ViewModels / Models / Services` 的职责边界和闭环在第 12 章展开
...
本章做“工程复盘版”总结，避免与第 2 章重复讲解细节。

详细原理请回看：

- 第 2 章的 [2.5 XAML + C++/WinRT 的协作方式](G:/code/guide/WinUI3/README.md)
- 第 2 章的 [2.6 `winrt::` 到底是什么](G:/code/guide/WinUI3/README.md)

这里保留一条最小闭环，帮助你在进入第 7～12 章前快速对齐模型：
...
1. **结构归 XAML**：布局、控件树、样式、资源在 XAML
2. **行为归 C++/WinRT**：事件、命令、异步、状态变更在 C++ 层
3. **状态不散落在控件里**：状态集中在 ViewModel，UI 通过绑定读取

如果这三条成立，后续的控件、布局、异步、导航、页面拆分就不会退化成“页面事件堆逻辑”。
...
## 14. 应用架构与交付：Assets、Manifest、MSIX
...
### 14.2 `Package.appxmanifest` 负责告诉系统“应用长什么样”
...
### 14.3 工程分层：Page / ViewModel / Model / Service
...
### 16.1 WinUI 3 仍然运行在传统 Windows 进程模型上
...
### 16.2 进程管理：WinUI 3 应用仍然能启动和管理其他程序
...
### 16.7 一个最重要的认知：WinUI 3 不是“逃离 Win32”，而是把它升级成现代工程模型
...
### 16.11 Future 模式：把“结果”当成一个可等待对象
...
### 16.12 Fork-Join 模式：递归拆分任务，再汇总结果
...
### 16.13 进程、线程、线程池、Future、Fork-Join 的关系
...
### 16.15 WinUI 3 应用中的内存管理：不是“离开操作系统”，而是“更严密的对象生命周期管理”
...
#### 16.15.2 WinUI 3 / C++/WinRT 的核心思路：RAII + 引用计数
...
#### 16.15.3 `winrt::com_ptr`：WinRT 对象不是裸指针，而是引用计数对象
...
#### 16.15.5 `std::shared_ptr` 与 `ViewModel`：共享状态要小心
...
#### 16.15.6 真实的内存管理原则：不要把 UI 和业务状态搞混
...
#### 16.15.7 一个最重要的认知：WinUI 3 的内存管理仍然是系统编程能力的一部分
...
### 16.16 WinUI 3 应用访问文件：不是“桌面 UI 以外的事”，而是系统能力的一部分
...
#### 16.16.1 典型策略：后台线程读取文件，UI 线程更新界面
...
#### 16.16.3 文件访问和内存管理的结合：读取数据后写回状态
...
### 16.18 WinUI 3 应用访问注册表：系统配置与本地状态管理
...
### 16.19 WinUI 3 应用读写 JSON 文件：结构化配置和数据载入
...
### 16.20 WinUI 3 应用读写 XML 文件：结构化文档和配置重用
...
### 16.21 真实工程里，注册表 / JSON / XML 是怎么协同的
...
5. ViewModel / 命令 / 数据绑定
6. 异步更新与后台任务
7. 资源与主题（Fluent Design / ThemeResource / ResourceDictionary）
8. 应用架构与交付（Assets / Package.appxmanifest / MSIX）
9. 一个真实功能型应用：任务管理器 / 设置页 / 日志页
10. OS 深入：进程、线程、任务队列、线程池、Future、文件/注册表/JSON/XML
