...
### 24.5.6 ContentDialog 的焦点纪律

对话框打开时**焦点自动落 DefaultButton**（回车即默认动作）——但焦点陷阱也是它的义务：Tab 在对话框内循环，不会跑到后面的遮罩内容。**打开前谁有焦点**会被记住，关闭后还回去（键盘流不断）。教学自检法：开对话框后按几下 Tab 再按 Esc——焦点回到调用前的控件即为正确。ScratchPad 的关闭确认（24.5）从页签 X 进入，Cancel 后焦点该回页签——微妙但真实可感。

### 24.5.7 遮罩的颜色语义

ContentDialog 的遮罩（Scrim）默认半透明黑——它不只是"挡视线"，是**模态的视觉宣告**：后面内容变暗=不可交互。自定义模板时别把遮罩删了（无遮罩的"模态"对话框，用户会去点后面的东西然后困惑为什么没反应）。Flyout 没有遮罩——这是两级打断在视觉上的直接表达（25.5.3 表的视觉列）。

## 24.6 练习与思考

1. 24.5.4 的排他律：连续快速关两个脏页签（两记 Ctrl+W + 页签 X），用 m_dialogUp 闸实现"第二个等待或退化为直接保存"。你选哪个？为什么？
2. 把未保存确认从 ContentDialog 降为 Flyout（24.5.5 的分界练习）：什么前提下这是对的？数据丢失风险怎么变？
3. DefaultButton 换 Secondary（Discard 为回车默认）——设计一个"危险默认"的正当场景，或论证它不存在。
