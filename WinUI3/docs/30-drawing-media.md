...
1. 给柱状图加 Y 轴刻度（30.5.3 的第一步）：四条 Line + TextBlock，与柱子同用 max 归一化公式。
2. 柱子加 hover 高亮（30.5.6 的命中测试）：Tapped 事件挂 Rectangle，状态行报"周三：96 次访问"。
3. 把柱子换成 Path 画的平滑曲线（面积图）——数据点到贝塞尔控制点的换算规则是什么？

### 30.5.7 图表的配色纪律

多系列柱状图的取色：**同亮度不同色相**（hsl 转 rgb：hue 均分、s/l 恒定）比手挑色和谐；单系列（本图）全用 accent——色相数=数据维度数是铁律（颜色是编码不是装饰）。色盲安全：红绿对（Sunset 红 vs Forest 绿）在色盲视野里撞色——用亮度区分（一深一浅）或直接加纹理（描边/图案）。**验证法**：截图转灰度看还能不能分辨系列——灰度下可分=真用了亮度通道。

## 30.7 上生产前的审查清单

- [ ] 数据归一化除以最大值（比例正确）
- [ ] 柱体沉底（VerticalAlignment=Bottom）
- [ ] 颜色是数据编码不是装饰（维度数=色相数）
- [ ] 灰度截图验证可分辨（色盲安全）
- [ ] 矢量 vs 位图按"内容跟谁走"选（30.5.4）
