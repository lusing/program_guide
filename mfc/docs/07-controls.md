...
**后台线程直接调 m_list 的方法**：跨线程访问 UI 控件是未定义行为，一律 PostMessage 给 UI 线程代做（第 12 章）。

## 6. 实战建议

- 报表类界面优先想 `CListCtrl + lParam 数据 + 列头排序` 这三件套；够用再考虑引入虚拟列表
- 控件封装成自己的小类（继承 CListCtrl，内嵌排序/数据管理），主窗口只管拼装——第 13 章统计面板示范了简化版
- 调试控件状态时用 `SPY++` 看通知流，比断点猜测快

---
上一章：[06 对话框](06-dialogs.md) ｜ 下一章：[08 通用对话框与文件 IO](08-common-dialogs.md)
