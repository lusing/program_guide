; ============================================================
; 文件: 10_sse_simd/avx2_int.asm                           [macOS 版]
; 指令: VPMULLD / VPSLLVD / VPERMD / VPBROADCASTD（AVX2）
; 描述: 整数运算从 128 位到 256 位 —— SSE 根本做不到的几件事
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/10_sse_simd/avx2_int.asm -o build/avx2_int.o
; 链接: clang -arch x86_64 build/avx2_int.o -o build/avx2_int
...
;
; ------------------------------------------------------------
; macOS 要点：整数结果用 %d 打印，参数走 rdi/rsi/rdx，不占 xmm；
