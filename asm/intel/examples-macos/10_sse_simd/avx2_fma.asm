; ============================================================
; 文件: 10_sse_simd/avx2_fma.asm                           [macOS 版]
; 指令: VFMADD231PS（FMA3）+ MULPS/ADDPS 对照
; 描述: 一条指令顶两条，而且只舍入一次 —— 差别在小数点后第 14 位
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/10_sse_simd/avx2_fma.asm -o build/avx2_fma.o
; 链接: clang -arch x86_64 build/avx2_fma.o -o build/avx2_fma
...
;
; ------------------------------------------------------------
; macOS 要点：这条测试用的是 float（单精度），打印前要 cvtss2sd 转 double，
