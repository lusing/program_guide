; ============================================================
; 文件: 10_sse_simd/avx_basics.asm                         [macOS 版]
; 指令: VADDPS / VMULPS / VZEROUPPER（AVX1，256 位 YMM）
; 描述: VEX 编码到底带来什么 —— 三操作数 + 一次算 8 个 float
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/10_sse_simd/avx_basics.asm -o build/avx_basics.o
; 链接: clang -arch x86_64 build/avx_basics.o -o build/avx_basics
...
;
; ------------------------------------------------------------
; macOS 要点：探测 AVX 不能只看 CPUID，还要看操作系统有没有放开 YMM 状态。
