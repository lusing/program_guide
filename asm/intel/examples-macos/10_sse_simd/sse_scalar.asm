; ============================================================
; 文件: 10_sse_simd/sse_scalar.asm                         [macOS 版]
; 指令: ADDSS / SUBSS / MULSS / ADDSD / MULSD
; 描述: 标量运算 —— 一次只算一个数，但快得没有对手
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/10_sse_simd/sse_scalar.asm -o build/sse_scalar.o
; 链接: clang -arch x86_64 build/sse_scalar.o -o build/sse_scalar
...
;
; ------------------------------------------------------------
; macOS 传参：%f 的值直接放 xmm0，eax 记「用了几个向量寄存器」。
