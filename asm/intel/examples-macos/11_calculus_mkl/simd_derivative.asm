; ============================================================
; 文件: 11_calculus_mkl/simd_derivative.asm                [macOS 版]
; 指令: MOVUPS / SUBPS / MULPS / ANDPS / MAXPS / SHUFPS
; 描述: SIMD 数值微分 —— 中心差分一次算 4 个点
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/11_calculus_mkl/simd_derivative.asm -o build/simd_derivative.o
; 链接: clang -arch x86_64 build/simd_derivative.o -o build/simd_derivative
...
; 依赖 _main 里已就绪的 r12 = d_arr、r13 = c_arr（都是被调用者保存寄存器）
; ------------------------------------------------------------
print_sample:
    push rbp
    mov rbp, rsp
