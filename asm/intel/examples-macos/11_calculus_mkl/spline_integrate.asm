; ============================================================
; 文件: 11_calculus_mkl/spline_integrate.asm               [macOS 版]
...
; 汇编: nasm -I lib -f macho64 examples-macos/11_calculus_mkl/spline_integrate.asm -o build/spline_integrate.o
; 链接: clang -arch x86_64 build/spline_integrate.o -o build/spline_integrate
...
;   **macOS 的 Accelerate.framework 没有对应的样条 API**
;   （vecLib 里有 vDSP 的二次插值 vDSP_vqint、以及 Quadrature 的自适应积分
;     quadrature_integrate，但没有 pp-form 样条构造。）
;   所以这里把样条**手写出来**，三段式：
;       ① 组装三对角方程组  ② Thomas 算法解 M_i = S''(x_i)  ③ 分段求值
;   手写反而更能看清 MKL 那条 API 背后到底在算什么，
;   而且这份代码在 Windows 上原样可编译 —— 没有任何平台依赖。
;
; ------------------------------------------------------------
...
section .data
    ...
    fmt_why2  db "注：macOS 的 Accelerate 没有 MKL DF 那样的样条 API，故此处手写。", 10, 0
