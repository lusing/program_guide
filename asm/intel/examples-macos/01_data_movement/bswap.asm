; ============================================================
; 文件: 01_data_movement/bswap.asm                         [macOS 版]
; 指令: BSWAP
; 描述: BSWAP 字节序反转 —— 大端/小端互换
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/01_data_movement/bswap.asm -o build/bswap.o
; 链接: clang -arch x86_64 build/bswap.o -o build/bswap
; 对照: examples/01_data_movement/bswap.asm
;
; x86 是小端机。网络字节序（big-endian）和 x86 内存字节序相反，
; 所以在 macOS 上写 socket / 二进制文件格式时，BSWAP 是最直接的转换手段。
