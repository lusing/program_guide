; ============================================================
; 文件: 02_arithmetic/div.asm                              [macOS 版]
; 指令: DIV
; 描述: 无符号除法 —— 32 位与 64 位，除之前必须清零高位寄存器
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/02_arithmetic/div.asm -o build/div.o
; 链接: clang -arch x86_64 build/div.o -o build/div
; 对照: examples/02_arithmetic/div.asm
;
; DIV 是「双宽度被除数」：EDX:EAX ÷ 源，商回 EAX，余数回 EDX。
; 所以做普通 32 位除法前一定要 xor edx,edx，否则余下的一半会把结果搅乱，
; 商超过 32 位时还会触发 #DE（在 macOS 上是 SIGFPE，直接被信号打死）。
; ============================================================
default rel
