; ============================================================
; 文件: 08_system_misc/cpuid_hybrid.asm                    [macOS 版]
; 指令: CPUID（页 0 / 页 1Ah / 页 7）
; 描述: 检测 Intel 大小核（P-Core / E-Core）与 AVX2 / AVX-512
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/08_system_misc/cpuid_hybrid.asm -o build/cpuid_hybrid.o
; 链接: clang -arch x86_64 build/cpuid_hybrid.o -o build/cpuid_hybrid
...
;
; 关键习惯：**先问最大页号，再决定要不要查**。
; 在 Haswell（i7-4770HQ）上实测过越界查页 1A：返回 EAX=0x00000007、
; EBX=ECX=0x00000340 —— 是别的页的遗留数据，不是 1A 的内容，也未定义。
; 不先看最大页号就查，等于拿垃圾数据做判断。
; 在两台旧 Intel 机器上都实测过：Ivy Bridge（i7-3520M）页 1A 不存在、AVX2 也不支持；
; Haswell（i7-4770HQ）页 1A 仍不存在但 AVX2 支持。两台都如实报 NO/YES，
; 这本身就是一次正确的「能力探测」示范 —— 不猜，问，然后按结果降级。
;
; ------------------------------------------------------------
; macOS 要点：CPUID 会踩 EBX，而 EBX 是被调用者保存寄存器，
; 取完所有 CPUID 必须还原，否则 _main 返回时 dyld 崩溃。
; ============================================================
default rel
