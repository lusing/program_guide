; ============================================================
; 文件: 05_control_flow/call_ret.asm                       [macOS 版]
; 指令: CALL / RET
; 描述: 自己定义子函数、按 ABI 传参、用 RET 返回
; 平台: macOS x86-64（Mach-O + System V AMD64）
; 汇编: nasm -I lib -f macho64 examples-macos/05_control_flow/call_ret.asm -o build/call_ret.o
; 链接: clang -arch x86_64 build/call_ret.o -o build/call_ret
; 对照: examples/05_control_flow/call_ret.asm
;
; 这是 Windows 和 macOS 差别最大的一个例子，两处都值得记住：
;
; 1) 参数寄存器
;      Windows: 第 1~4 个参数 rcx rdx r8 r9（第 5 个起走栈）
;      macOS  : 第 1~6 个参数 rdi rsi rdx rcx r8 r9（第 7 个起走栈）
;
; 2) 能不能用 RET 从 main 返回
;      Windows 版用 link /entry:main 指定入口，栈上没有返回地址，
;      所以 main 只能用 ExitProcess 退出；
;      macOS 版 _main 是 libSystem 启动代码 call 进来的普通函数，
