; ============================================================
; 文件: 10_sse_simd/sse_arithmetic.asm                     [Linux 版]
; 指令: ADDPS / SUBPS / MULPS / DIVPS
; 描述: 一条指令算四个 —— SIMD 的正题
; 平台: Linux x86-64（ELF + System V AMD64）
; 汇编: nasm -I lib -f elf64 examples-linux/10_sse_simd/sse_arithmetic.asm -o build/sse_arithmetic.o
; 链接: gcc -no-pie build/sse_arithmetic.o -o build/sse_arithmetic
...
;
; ------------------------------------------------------------
; Linux 要点：`"  [%d] = %f"` 的参数是 rdi / esi / xmm0 / eax=1。
