; ============================================================
; 文件: 03_logic_bitwise/and_or_xor_not.asm                [Linux 版]
; 指令: AND / OR / XOR / NOT
; 描述: 基本逻辑运算 —— 掩码提取、置位、翻转、取反
; 平台: Linux x86-64（ELF + System V AMD64）
; 汇编: nasm -I lib -f elf64 examples-linux/03_logic_bitwise/and_or_xor_not.asm -o build/and_or_xor_not.o
; 链接: gcc -no-pie build/and_or_xor_not.o -o build/and_or_xor_not
; 对照: examples/03_logic_bitwise/and_or_xor_not.asm
;
; Windows 版开头有一段 `and rsp,-16`，是为了在压影子空间之前自己保证 16 字节对齐。
; Linux 走 SysV，压过 rbp 之后栈就已经对齐，不需要这一手（写了也不算错）。
; ============================================================
default rel
