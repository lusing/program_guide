; ============================================================
; 文件: 08_system_misc/rdtsc.asm                           [Linux 版]
; 指令: RDTSC / RDTSCP
; 描述: 读时间戳计数器 —— 用 CPU 自带的「纳秒级秒表」量代码
; 平台: Linux x86-64（ELF + System V AMD64）
; 汇编: nasm -I lib -f elf64 examples-linux/08_system_misc/rdtsc.asm -o build/rdtsc.o
; 链接: gcc -no-pie build/rdtsc.o -o build/rdtsc
...
;
; ------------------------------------------------------------
; Linux 要点：rdtsc 不碰任何通用寄存器，所以移植只改调用约定。
