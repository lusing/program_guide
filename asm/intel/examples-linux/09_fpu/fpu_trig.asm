; ============================================================
; 文件: 09_fpu/fpu_trig.asm                                [Linux 版]
; 指令: FLDPI / FSIN / FCOS / FPTAN / FIDIV
; 描述: x87 自带的三角函数 —— 四行指令算完 sin/cos/tan
; 平台: Linux x86-64（ELF + System V AMD64）
; 汇编: nasm -I lib -f elf64 examples-linux/09_fpu/fpu_trig.asm -o build/fpu_trig.o
; 链接: gcc -no-pie build/fpu_trig.o -o build/fpu_trig
...
; ------------------------------------------------------------
; Linux 差异仅在参数传递：浮点结果 `movsd xmm0, [result]` + `mov eax,1`。
; ============================================================
default rel
