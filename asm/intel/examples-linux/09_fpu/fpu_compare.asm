; ============================================================
; 文件: 09_fpu/fpu_compare.asm                             [Linux 版]
; 指令: FCOMI / FCOM + FNSTSW + SAHF
; 描述: 浮点比较 —— 新老两条路，一条直通 EFLAGS，一条要绕状态字
; 平台: Linux x86-64（ELF + System V AMD64）
; 汇编: nasm -I lib -f elf64 examples-linux/09_fpu/fpu_compare.asm -o build/fpu_compare.o
; 链接: gcc -no-pie build/fpu_compare.o -o build/fpu_compare
...
;
; ------------------------------------------------------------
; Linux 要点：这段逻辑与平台无关，只把 printf 参数从
