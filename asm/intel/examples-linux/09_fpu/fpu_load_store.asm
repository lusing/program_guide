; ============================================================
; 文件: 09_fpu/fpu_load_store.asm                          [Linux 版]
; 指令: FLD / FILD / FST / FSTP
; 描述: 数据和 x87 栈之间怎么来回搬 —— 以及 FST 与 FSTP 的一字之差
; 平台: Linux x86-64（ELF + System V AMD64）
; 汇编: nasm -I lib -f elf64 examples-linux/09_fpu/fpu_load_store.asm -o build/fpu_load_store.o
; 链接: gcc -no-pie build/fpu_load_store.o -o build/fpu_load_store
...
;
; ------------------------------------------------------------
; Linux 传浮点参数：只要 `movsd xmm0, [result]` + `mov eax,1`。
