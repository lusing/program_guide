; ============================================================
; rdtsc.asm - RDTSC时间戳计数器
; ============================================================
; 演示:
;   - rdtsc    读取时间戳计数器到 EDX:EAX
;   - rdtscp   序列化版本(等待之前的指令完成)
;   - 合并EDX:EAX为64位值: shl rdx,32; or rax,rdx
;   - 测量一小段代码的执行周期
; ============================================================
...
section .data
    fmt_ts1     db "RDTSC timestamp #1: %llu", 10, 0
    fmt_ts2     db "RDTSC timestamp #2: %llu", 10, 0
    fmt_start   db "Loop start timestamp: %llu", 10, 0
    fmt_end     db "Loop end timestamp:   %llu", 10, 0
    fmt_cycles  db "Cycles elapsed (1000 NOP loop): %llu", 10, 0
    fmt_rdtscp  db "RDTSCP timestamp (serialized): %llu", 10, 0
