; ============================================================
; fpu_add_sub.asm - FADD/FSUB 浮点加减法
; ============================================================
; 演示:
;   - faddp  ST0 = ST1 + ST0, 然后弹出 (ST1成为新的ST0)
;   - fsubp  ST0 = ST1 - ST0, 然后弹出
;   - fadd qword [mem]  ST0 += [mem] (不弹出, 直接加内存中的值)
;
; 计算: 3.14 + 2.72 = 5.86
;       10.5 - 3.7  = 6.8
; ============================================================
...
section .data
    align 8
    a_val   dq 3.14            ; 加法操作数1
    b_val   dq 2.72            ; 加法操作数2
    c_val   dq 10.5            ; 减法操作数1
    d_val   dq 3.7             ; 减法操作数2
    result  dq 0.0             ; 结果存储

    fmt_add  db "FADDP: 3.14 + 2.72 = %f", 10, 0
    fmt_sub  db "FSUBP: 10.5 - 3.7 = %f", 10, 0
    fmt_addm db "FADD [mem]: 3.14 + 2.72 = %f  (ST0 += memory)", 10, 0
...
main:
    ...
    ; -------------------------------------------------------
    ; 3. FADD qword [mem] - 直接加内存中的值
    ;    操作: ST0 += [mem] (不改变栈深度)
    ;    与FADDP不同，不弹出，直接将内存值加到ST0
    ; -------------------------------------------------------
    fld qword [a_val]          ; ST0 = 3.14
    fadd qword [b_val]         ; ST0 = 3.14 + 2.72 = 5.86 (直接加内存值)
    fstp qword [result]        ; 存储结果, 栈空
