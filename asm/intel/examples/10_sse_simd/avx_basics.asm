; ============================================================
; 文件: 10_sse_simd/avx_basics.asm                       [Windows 版]
; 指令: VADDPS / VMULPS / VZEROUPPER（AVX1，256 位 YMM）
; 描述: VEX 编码到底带来什么 —— 三操作数 + 一次算 8 个 float
; 平台: Windows x86-64（COFF + Win64 ABI）
; 汇编: nasm -f win64 examples/10_sse_simd/avx_basics.asm -o build/avx_basics.obj
; 链接: link /subsystem:console /entry:main build/avx_basics.obj msvcrt.lib legacy_stdio_definitions.lib kernel32.lib
; 对照: examples-macos/10_sse_simd/avx_basics.asm ｜ examples-linux/10_sse_simd/avx_basics.asm
;
; ------------------------------------------------------------
...
;
; ------------------------------------------------------------
; Windows 要点：探测 AVX 不能只看 CPUID，还要看操作系统有没有放开 YMM 状态。
;   页 1 ECX[27] = OSXSAVE、ECX[28] = AVX；
;   再 XGETBV 读 XCR0，要求 bit1|bit2 = 11 —— 否则 ymm 指令直接 #UD。
; 这部分代码三个平台完全一样，CPUID / XGETBV 是 CPU 给的，与操作系统无关。
; 真正要改的是调用约定：
;   - printf 第 1 个参数在 rcx、第 2 个在 rdx；
;   - **浮点参数既要写 xmm，又要同时写进对应的整数槽**（下例用 r8），
;     MSVC 的 printf 才读得到；
;   - 调用前要留出 32 字节影子空间。
