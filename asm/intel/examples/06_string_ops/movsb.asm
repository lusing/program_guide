; ============================================================
; movsb.asm - 字符串移动 (MOVS/MOVSB)
; ============================================================
; 演示:
;   - movsb     移动单字节 (DS:RSI -> ES:RDI), 指针自动+1
;   - rep movsb 重复移动整个字符串 (类似 memcpy)
;   - CLD       清除方向标志 (DF=0, 指针向前递增)
;   - STD       设置方向标志 (DF=1, 指针向后递减) - 本示例用CLD
;
; Windows x64 注意:
;   RDI 和 RSI 是 non-volatile (被调用者保存) 寄存器
;   使用前必须 push 保存, 使用后 pop 恢复
; ============================================================
...
section .data
    ...
    fmt_single db "1. movsb: copied 1 byte -> dest[0] = '%c' (0x%02X)", 10, 0
    fmt_full   db '2. rep movsb: copied %lld bytes -> dest = [%s]', 10, 0
...
main:
    ...
    ; 打印移动的单个字节
    movzx r8, byte [dest_buf]             ; R8 = 字符的ASCII值
    movzx rdx, byte [dest_buf]            ; RDX = 字符(用于%c)
    lea rcx, [fmt_single]
    ; RDX 已设置, R8 已设置
    call printf
    ...
    mov byte [dest_buf + src_len], 0      ; 添加 null 终止符
