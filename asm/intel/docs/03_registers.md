...
| YMM0–YMM15 | 256 位 | AVX/AVX2 | 256 位 SIMD（XMM 为其低 128 位），要 32 字节对齐；VEX 编码下三操作数、源不被破坏 |
| ZMM0–ZMM31 | 512 位 | AVX-512 | 512 位 SIMD（需特定 CPU 支持，本指南不涉及） |

XMM0 也用于浮点函数的返回值。详见 `10_sse_simd` 类别示例。
AVX / AVX2 / FMA 的用法 —— VEX 前缀、`ymm` 的 32 字节对齐要求、
以及「用完 YMM 别忘了 `vzeroupper`」的原因 —— 见 [第 12 章 SIMD 与 AVX](12_simd_avx.md)。

> 三个宽度是**层层包含**的：`xmm0` 就是 `ymm0` 的低 128 位，`ymm0` 又是 `zmm0` 的低 256 位。
> 写窄不写宽：`vmovaps xmm0, xmm1` 只动低 128 位，高 128 位**保持原值不变**（不是清零），
> 但一旦用 legacy SSE 指令写 `xmm0`，`ymm0` 的高 128 位就会被清零 —— 这正是 SSE 与 AVX
> 混用时性能出坑的来源，详见第 12 章「YMM 与 vzeroupper」。
