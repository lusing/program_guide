...
10. **等值元素顺序无语言保证**：跨实现/跨版本/**跨平台**别赌 sort 稳定，要稳定把位置并进键——同一个 `sort { |a, b| b[1] <=> a[1] }`，tom/anna 同为 92 分时 macOS 排 tom 在前、Windows 排 anna 在前（4.0.7 双平台实测），比较器没写平局裁决就只断言不受平局影响的部分（9.4、11.8）。
