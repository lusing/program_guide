...
$erlcCandidates = @(
    "G:\scoop\apps\erlang\current\bin\erlc.exe",
    "G:\scoop\apps\erlang\current\erts-17.0.6\bin\erlc.exe"
)

$erlc = $erlcCandidates | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
if (-not $erlc) {
    throw "未找到 erlc.exe，请检查 Erlang 安装路径。"
}
...
if ($All) {
    $files = Get-ChildItem -LiteralPath $examplesDir -Filter "*.erl" | Sort-Object Name
    if ($files.Count -eq 0) {
        throw "examples 目录下没有 .erl 示例文件。"
    }
...
Write-Host "  .\build.ps1 -File <name.erl> 编译单个示例"
