%% ============================================================
%% 23 - 应用（Application）：把一整棵进程树打包成一个可启动的单元
%%
%%    前面 21/22 章讲的是「怎么组织进程」。这一章讲的是**怎么把它们装起来**。
%%
%%    OTP 的组织层次是：
...
%%
%%    这一章会真的启动 examples/kvapp/ 里那个最小 OTP 应用：
...
%%
%%    编译：
%%      erlc -Werror -Wall -o build/ebin examples/kvapp/*.erl
%%      cp examples/kvapp/kvapp.app build/ebin/     ← .app 不是代码，erlc 不认，要手工拷
%%    运行：
%%      erl -noshell -pa build/ebin -run '23-application' main -s init stop
%%
%%    注意：本示例在末尾会 unload 掉 kvapp，跑完环境是干净的。
%% ============================================================
-module('23-application').
...
layers() ->
    io:format("== 1) 三层结构 ==~n"),
    Rows = [{"进程", "spawn 出来的东西；不共享内存，只靠消息通信（18/19 章）"},
...
pitfalls() ->
    ...
    Rows =
        [{"忘了把 <应用>.app 拷到代码路径",
          ...
          "erlc 只编 .erl，.app 是数据文件，构建脚本里必须显式拷（见 run-all.sh）"},
    ...
    case application:load(kvapp) of
        ...
        {error, Reason} ->
            io:format("~n找不到 kvapp.app（~p）。~n"
                      "请先运行 ./run-all.sh —— 它会把 examples/kvapp/*.app 拷到 build/ebin/。~n",
                      [Reason]),
            halt(1)
    end.
