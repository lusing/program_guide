...
%%
%% 编译：
%%   erlc -Werror -Wall -o build/14_links examples/14_links/14_links.erl
%% 运行：
%%   erl -noshell -pa build/14_links -run '14_links' main -s init stop
%% ============================================================
-module('14_links').
...
main() ->
    ...
    %% 带时间戳和 pid（见第 12 章实测）。本示例要可重复输出，先摘掉它；
...
why_supervision() ->
    ...
    io:format("  ~ts~n", ["  第 15 章 gen_server、第 16 章 supervisor、第 17 章 application。"]),
    ok.
