...
%%
%% 编译：
%%   erlc -Werror -Wall -o build/ebin examples/24-ets.erl
%% 运行：
%%   erl -noshell -pa build/ebin -run '24-ets' main -s init stop
%% ============================================================
-module('24-ets').
...
%% 这是让 ETS 表活过「worker 重启」的标准手段（第 28 章会用到）。
ownership() ->
...
options_and_usage() ->
    ...
    Tips = [{"一个进程的高频本地缓存", "还是用 map（ETS 有额外开销）"},
            ...
            {"数据必须跨进程重启还在", "ETS + heir（第 28 章），或者干脆用数据库"},
