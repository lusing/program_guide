...
%%  26) 定时器、时间与系统限制
%%
%%  编译：erlc -Werror -Wall -o build/ebin examples/26-timers-limits.erl
%%  运行：erl -noshell -pa build/ebin -run '26-timers-limits' main -s init stop
%%
%%  这一章讲三件事：
...
system_limits() ->
    ...
    io:format("  上限算出来的，同一个仓库用 ./run-all.sh 跑和用 pwsh ./build.ps1 跑~n"),
