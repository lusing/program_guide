%% ---------------------------------------------------------------------------
%%  19_files —— 文件 I/O 与二进制序列化
%%
%%  编译：erlc -Werror -Wall -o build/19_files examples/19_files.erl
%%  运行：erl -noshell -pa build/19_files -run '19_files' main -s init stop
%%
%%  这一章解决三件实际的事：
%%    1. 文件和目录怎么读写、路径怎么拼；
...
%%
%%  本章会用到的临时目录固定在 build/erl-demo-19/，开头重建、结尾删掉，
%%  所以可以反复跑，输出稳定。
%% ---------------------------------------------------------------------------
-module('19_files').
...
-export([main/0, encode/3, decode/1]).
...
paths() ->
    ...
    d("filename:nativename（Windows 上分隔符变反斜杠）",
...
streaming_io() ->
    ...
    d("file:pread(Fd, 0, 2)（定位读）", file:pread(Fd, 0, 2)),
    d("  实测：pread 之后 position 变了（Windows/OTP29 会移动偏移；"
      "macOS 上不动——别依赖它保持位置）",
