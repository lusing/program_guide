...
%% 03_types —— 数值、原子与项序
%%
%%    整数是任意精度的、浮点溢出抛 badarith 而不是 inf、
%%    div/rem 一律向零截断、== 与 =:= 是两套比较、
%%    原子表只涨不回收——这些点全部实测过再信。
%%
%% 编译：
%%   erlc -Werror -Wall -o build/03_types examples/03_types/03_types.erl
%% 运行：
%%   erl -noshell -pa build/03_types -run '03_types' main -s init stop
%% ============================================================
-module('03_types').

-export([main/0, fact/1, safe_atom/1, raises/2]).
...
bit_ops() ->
    ...
    d("拆字节：0xABCD 的高/低字节", {16#ABCD bsr 8, 16#ABCD band 16#FF}),
    ok.
...
rounding() ->
    ...
    d("io_lib:format(\"~.0f\", [2.5]) 会抛",
      try io_lib:format("~.0f", [2.5]) catch _:R -> R end),
    ok.

%% 7) 比较与全序
%% ------------------------------------------------------------
compare() ->
    io:format("~n== 7) 比较运算符与项序 ==~n"),
