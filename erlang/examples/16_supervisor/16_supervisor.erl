%% ============================================================
%% 16_supervisor —— supervisor：把「崩了就重启」变成声明式的
%%
%%    第 14 章我们看到，手写 link + trap_exit 很快就不够用了：
...
%%
%% 编译：
%%   erlc -Werror -Wall -o build/16_supervisor examples/16_supervisor/16_supervisor.erl
%% 运行：
%%   erl -noshell -pa build/16_supervisor -run '16_supervisor' main -s init stop
%% ============================================================
-module('16_supervisor').
...
strategy_table() ->
    ...
    %% 而一个汉字占两列，表格必然错位（第 12 章有按显示宽度补空格的写法）。
    %% 这里干脆一行一条，省掉对齐问题。
