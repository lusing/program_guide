...
%%  27) 日志（logger）与可观测性
%%
%%  编译：erlc -Werror -Wall -o build/ebin examples/27-logger.erl
%%  运行：erl -noshell -pa build/ebin -run '27-logger' main -s init stop
%%
%%  ⚠ 本章的实现方式说明（很重要）：
%%  日志是由 **handler 所在的另一个进程** 写出去的，它和本进程的 io:format
%%  之间没有顺序保证 —— 实测中同一份代码，日志行会插在 io:format 行的任意位置。
%%  为了让输出可复核，本章的每一轮演示都：
%%      1) 把日志写进 /tmp/erl-demo-27/log.txt（logger_std_h 的 {file, Name}）
...
-define(DIR, "/tmp/erl-demo-27").
-define(LOGFILE, "/tmp/erl-demo-27/log.txt").
-define(H, demo_h).
