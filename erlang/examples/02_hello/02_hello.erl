...
%%    必须写成「引号原子」，所以模块名是 '02_hello'。
%%    你自己的工程模块用普通名字（hello、kv_store……）即可。
%%
%% 编译：
%%   erlc -Werror -Wall -o build/02_hello examples/02_hello/02_hello.erl
%% 运行：
%%   erl -noshell -pa build/02_hello -run '02_hello' main -s init stop
%% 交互式：
%%   erl -pa build/02_hello
%%   1> '02_hello':greet("世界").
%%
...
-export([main/0, greet/1, render/2, show_module_info/0]).

%% 1) 最小可运行模块
%% ------------------------------------------------------------
%% erl 的 -run '02_hello' main 会调用 main/0（参数个数为 0）。
main() ->
    ...
    %% 所以只断言「至少 1 个」，不把具体数字打进输出 ——
    %% 否则 build.ps1 的「两种调度器配置输出必须逐字节一致」就过不了。
...
greet(Name) when is_list(Name) ->
    %% ~ts 是「按 unicode 打印」，中文必须用它（~s 只认 latin1，见 09 章）
    io:format("  你好，~ts！~n", [Name]);
greet(Other) ->
    io:format("  期望字符串，收到 ~p~n", [Other]).

%% 纯函数：用指定格式渲染**一个**值，返回结果而不打印。
%% 展示用的 dir/3 靠它工作，测试模块也直接测它。
render(Fmt, Value) ->
...
show_directives() ->
    ...
    %% 正文里的波浪号要写两遍（~~ 渲染成字面 ~，不消耗参数）
    io:format("  ~-8ts ~ts  ->  ~ts~n",
              ["~~", "正文中的字面波浪号",
               lists:flatten(io_lib:format("100~~%", []))]),
    ok.
...
show_module_info() ->
    ...
    Which = code:which('02_hello'),
    io:format("  code:which 的文件名 = ~ts~n", [filename:basename(Which)]),
...
dir(Fmt, Desc, Value) ->
    io:format("  ~-8ts ~ts  ->  ~ts~n", [Fmt, Desc, render(Fmt, Value)]).
