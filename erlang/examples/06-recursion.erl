...
%%    具体数值随实现变化，所以示例里同时打印一个与实现无关的布尔结论。
%%
%% 编译：
%%   erlc -Werror -Wall -o build examples/06-recursion.erl
%% 运行：
%%   erl -noshell -pa build -run '06-recursion' main -s init stop
%% ============================================================
-module('06-recursion').

-export([main/0, fact/1, fib/1, sum_to/1, depth_count/1, is_even/1]).
...
tail_call_cost() ->
    ...
    d("递归深度", Depth),
    d("非尾递归峰值内存 (words)", NonTail),
    d("尾递归峰值内存 (words)", Tail),
    d("两者结果相同", count_down(Depth) =:= count_down_tail(Depth)),
    %% 与实现无关的结论：只断言「相差一个数量级以上」
    d("非尾递归内存 > 尾递归内存 * 10", NonTail > Tail * 10),
    ok.
