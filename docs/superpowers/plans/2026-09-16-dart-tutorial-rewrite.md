...
- [x] **Step 2: 烟测脚本（此时 examples 不存在，走报错/用法路径）**
...
- [x] **Step 1: 写 examples/02_hello.dart**
...
- [x] **Step 2: 写 examples/03_variables.dart**
...
- [x] **Step 3: 写 examples/04_control_flow.dart**
...
- [x] **Step 4: 写 examples/05_functions.dart**
...
- [x] **Step 1: 写 examples/06_collections.dart**
...
- [x] **Step 2: 写 examples/07_classes.dart**
...
- [x] **Step 3: 写 examples/08_inheritance.dart**
...
- [x] **Step 1: 写 examples/09_mixins_modifiers.dart**
...
- [x] **Step 2: 写 examples/10_null_safety.dart**
...
- [x] **Step 3: 写 examples/11_generics.dart**
...
- [x] **Step 1: 写 examples/12_exceptions.dart**
...
- [x] **Step 2: 写 examples/13_records_patterns.dart**
...
- [x] **Step 3: 写 examples/14_extensions.dart**
...
- [x] **Step 1: 写 examples/15_async.dart**
...
- [x] **Step 2: 写 examples/16_streams.dart**
...
- [x] **Step 3: 写 examples/17_isolates.dart**
...
- [x] **Step 1: 写 examples/18_files_json_http.dart**
...
- [x] **Step 3: 写 examples/19_testing/lib/cart.dart**
...
- [x] **Step 4: 写 examples/19_testing/test/cart_test.dart**
...
- [x] **Step 3: 写 lib/src/parser.dart**
...
- [x] **Step 4: 写 lib/src/storage.dart**
...
- [x] **Step 5: 写 lib/todo.dart 与 bin/todo.dart**
...
- [x] **Step 6: 写 test/todo_test.dart**
...
- [x] **Step 2: 写 02-hello.md**（约 110 行）：
...
- [x] **Step 3: 写 03-values.md**（约 170 行）：
  1. `# 03 · 变量与内置类型：一切皆对象`
...
- [x] **Step 4: 写 05-functions.md**（约 160 行）：
  1. `# 05 · 函数：参数三形态与一等公民`
...
- [x] **Step 1: 写 04-control-flow.md**（约 150 行）：
  1. `# 04 · 控制流：从语句到表达式`
...
- [x] **Step 2: 写 06-collections.md**（约 190 行）：
...
- [x] **Step 3: 写 07-classes.md**（约 200 行）：
  1. `# 07 · 类与对象：构造函数的六种形态`
...
- [x] **Step 4: 写 08-inheritance.md**（约 160 行）：
  1. `# 08 · 继承、抽象与隐式接口`
...
- [x] **Step 1: 写 09-mixins-modifiers.md**（约 180 行）：
...
- [x] **Step 2: 写 10-null-safety.md**（约 170 行）：
...
- [x] **Step 3: 写 11-generics.md**（约 150 行）：
  1. `# 11 · 泛型：参数化类型与协变边界`
...
- [x] **Step 4: 写 12-exceptions.md**（约 160 行）：
...
- [x] **Step 1: 写 13-records-patterns.md**（约 220 行，本教程重点章）：
...
- [x] **Step 2: 写 14-extensions.md**（约 160 行）：
  1. `# 14 · 扩展：不改源码地"加方法"`
...
- [x] **Step 3: 写 15-async.md**（约 180 行）：
...
- [x] **Step 4: 写 16-streams.md**（约 180 行）：
  1. `# 16 · Stream：异步的数据序列`
...
- [x] **Step 1: 写 17-isolates.md**（约 170 行）：
...
- [x] **Step 2: 写 18-files-json-http.md**（约 180 行）：
...
- [x] **Step 3: 写 19-testing.md**（约 170 行）：
...
- [x] **Step 4: 写 20-todo.md**（约 200 行，实战章）：
  1. `# 20 · 实战：CLI 待办管理器`
...
- [x] **Step 1: 写 test/guide_test.dart**（自包含，不 import 示例文件）
...
- [x] **Step 3: 重写 README.md**（对齐 fsharp README 格式，约 70 行）：
...
- [x] **Step 4: 写 CHEATSheet.md**（约 180 行，速查非教程）：
...
| 位置 | 偏差 | 原因 |
|---|---|---|
| Task 1 根 analysis_options.yaml | 增加 `file_names: false`（带注释） | 示例以章号数字前缀命名（02_hello.dart），数字开头必触发该 lint；这是"章号=示例号"约定的刻意选择 |
| Task 3 示例 03/04 | `count is num` 行加 ignore；`${parity}数` 去花括号 | `unnecessary_type_check`/`unnecessary_brace_in_string_interps` |
| Task 5 示例 10/11 | `?.`/`!`/判空演示改为经函数取值（fetchSubtitle/fetchLocal）；`is List<int>` 改经父类引用判断 | 分析器流分析对"静态可判空"的变量报死代码/无效断言 |
| Task 6 示例 13/14 | 命名字段 record 改用 `(x: px, y: py)` 解构（位置解构不匹配命名字段是真编译错误）；`String? maybe = null` 去初始化；typedef 闭包赋值加 ignore | 编译错误与两条 lint |
| Task 8 19_testing | `parse` 静态方法从 Cart 移入 CartItem | 测试调用形态 CartItem.parse 更合语义 |
| Task 15 guide_test.dart | 空安全用例改经函数来源；异常用例改 `throwsFormatException`；协变断言加 ignore | 同"流分析太聪明"系列；20_todo lib/todo.dart 的 `///` 改 `//`（dangling_library_doc_comments） |

全部偏差不改变章节结构、小节编号与教学语义；终验 `-All` 全绿（analyze 零告警、17 示例运行、AOT、+6/+9/+12 测试）。
