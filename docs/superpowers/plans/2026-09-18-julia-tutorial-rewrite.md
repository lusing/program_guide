...
0. **科学计算改版（用户反馈二，2026-09-18）**：用户指出整份教程"通用语言味太重、缺科学计算与
   数值性能"。结构重排（保持 24 章）：新增 ⭐13_linalg（分解复用/最小二乘/SVD 条件数/SparseArrays/
   BLAS 线程）与 ⭐21_randomstats（Random 子流/描述统计/蒙特卡洛/CLT/置信区间覆盖率）；03 扩为
   "数值类型与数值稳定"（抵消/ulp/Kahan/稳定求根）；13 异常并入新 22_errdebug、20+21 合并为
   20_concurrency（-t 4）、原 22 ccall 顺移 23_ccall。新增实测坑：**`sum` 舍入随编译旗标变**
   （--check-bounds=yes 下归约路径不同，[1e16; ones(1e4)] 差 38↔626）；**`sign(0)==0`**（稳定
   求根 b=0 时 c/q 得 Inf）；**循环内新建同种子 Xoshiro = 同一批样本**（std≈0 暴露）；**sprand
   均匀采样约 13% 行为空 → 奇异**（教学矩阵加对角占优项）；`rand(2.0..3.0)` 不存在（`..` 非
   Base）；ℯ 是 Base 导出、MathConstants 要显式引入；@assert 与 ≈ atol= 再踩两次（写成 isapprox）。

0.5. **24 章改版（用户反馈一）**：初版 24 章为"迷你 grep"（与 cpp20/zig 对齐）；交付后用户指出——
