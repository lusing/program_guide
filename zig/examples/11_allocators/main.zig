...
// ═══ 11.1 分配器哲学：分配器是第一个参数（用谁给的，就用谁）
fn dupString(allocator: std.mem.Allocator, s: []const u8) ![]u8 {
    return allocator.dupe(u8, s);
}

// ═══ 11.4 传参惯例：库函数只收分配器，挑哪种策略是调用方的事
...
pub fn main() !void {
    // ═══ 11.3 选型表 · page_allocator：直接向 OS 要页（最底层，每次分配整页开销）
    {
    ...
    // ═══ 11.3 选型表 · DebugAllocator（旧名 GPA）：带泄漏检测的调试堆（接口解剖的 alloc/free/dupe 都在这里用上）
    {
    ...
    // ═══ 11.3 选型表 · Arena：一次 init，批量分配，deinit 统一回收
    {
    ...
    // ═══ 11.3 选型表 · FixedBuffer：拿一块缓冲当堆（嵌入式/热路径/无堆环境）
    {
    ...
    // ═══ 11.3 选型表 · smp_allocator：多线程/发布版的系统级选择
    {
    ...
    // ═══ 11.5 内存布局基本功：栈优先（长度已知的不碰堆；undefined = 马上要整个覆写）
