...
```bash
dmd -run main.d           # 编译 + 立即运行（不落盘 exe）
dmd main.d                # 产 main.exe / main.obj
...
这是本教程 build.ps1 分两层验证的原因：一层 `-unittest -run` 验证测试，一层常规编译验证 main。
...
3. **obj 文件落 CWD**：`dmd main.d` 在当前目录留 `main.obj`——用 `-od目录` 指到别处（build.ps1 已处理）。
