...
全程断言）。每章"读讲解 → 跑示例 → 改代码再跑"，全部 18 个示例在 **macOS 与 Linux（双通道）及
Windows（解释通道，能力缺口自动跳过）** 实机验证通过。
...
> 本教程所有代码在 **Algol 68 Genie 3.13.3** 实测——**macOS（MacPorts + clang 后端）、Linux（官网
> 源码构建，双通道）** 与 **Windows 11（scoop，解释通道）** 三平台；每章末"坑位清单"收录版本差异与实测陷阱——
> [CHEATSheet.md](CHEATSheet.md) 汇总语法速查 + 约 **130 条实测坑位**，
> [docs/20-pitfalls.md](docs/20-pitfalls.md) 是完整分类索引。
...
| 章 | 主题 | 示例 |
|---|---|---|
| [01 全景与工具链](docs/01-overview.md) | Algol 68 家族史、a68g 定位（解释器+C 后端）、上戳写法、编译模型、验证方法论、macOS 链接坑 + Windows 三大缺口 | — |
...
| [17 并行](docs/17-parallel.md) ⭐ | PAR 并列子句、汇合、SEMA `LEVEL n` 临界区、生产者-消费者握手、竞态坑、确定性设计（Windows 构建无 PAR，自动跳过） | `17_parallel` |
...
- **Algol 68 Genie 3.13.3**（`a68g`）：macOS 走 MacPorts `/opt/local/bin/a68g`（`port install
  ...
  代码运行；Linux 用发行版包或官网构建。官方主页 <https://algol68genie.nl/>。
- **Windows**：`scoop install algol68g`（`%SCOOP%\apps\algol68g\current\bin\a68g.exe`）。官方构建
  **三大缺口**（实测）：① **无 C 后端**——`-O2`/`--compile`/`--optimise` 报
  `not implemented for this platform`；② **未编入 parallel-clause**——含 `PAR` 的源码语法层即拒
  （17 章示例跳过）；③ **INT 是 64 位**（macOS 构建是 32 位）——勿硬编码 `max int`。
  验证脚本开头用微型探针探测 C 后端与 PAR，缺口项自动 `[SKIP]` 并计入「平台跳过」，详见
  [docs/01 §9.2](docs/01-overview.md)。
- 源码一律 **UTF-8、上戳写法**（关键字全大写，见 [docs/01](docs/01-overview.md)），扩展名 `.a68`。
- 脚本按 **环境变量 `A68G` → 固定路径（含 `%SCOOP%`/`~/.scoop`）→ PATH** 顺序探测，不硬编码。
...
无 C 后端的构建（官方 Windows 版）上，脚本探针检测后 release 通道与含 PAR 的 17 章示例自动
`[SKIP]`（计入「平台跳过」，不计失败），验证收缩为单通道四条判定。
