...
全程断言）。每章"读讲解 → 跑示例 → 改代码再跑"，全部 18 个示例在 **macOS（双通道）与
Windows（解释通道，能力缺口自动跳过）** 两台实机验证通过。
...
> 本教程所有代码在 **Algol 68 Genie 3.13.3** 实测——**macOS（MacPorts + clang 后端，双通道）**
> 与 **Windows 11（scoop，解释通道）** 双平台；每章末"坑位清单"收录版本差异与实测陷阱——
