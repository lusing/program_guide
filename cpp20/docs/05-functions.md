...
void double_it(int x) {
    x *= 2;  // 改的是副本，外面看不见
    (void)x; // 表明"改完不用"是刻意演示：clang 会报 -Wunused-but-set-parameter
}
```

C++ 参数默认**按值拷贝**——函数拿到的是快照，改了也白改。这不是缺陷而是默认安全：调用方永远不被意外篡改。想要"改得动"或"不想拷贝大对象"，看下两节。

> 顺带一个跨编译器的观察：`-Wunused-but-set-parameter` 只有 clang 默认报（GCC、MSVC 都不报）。所以"我这台机器上零告警"从来不是"零告警"的证据——本教程的双编译器校验就是冲着这类差异去的。
