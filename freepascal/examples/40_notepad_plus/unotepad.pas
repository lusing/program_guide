...
begin
  ...
  Result.Encoding := encUtf8;
  // 编码坑（25 章）：显式 UTF-8 读，默认走系统 ANSI 会乱中文
...
var
  R: TModalResult;
begin
  // 脏文档问一句；干净直接关（25 章 ModalResult 语义的纯逻辑核）
...
begin
  MessageDlg('记事本+' + #10 + 'FreePascal/Lazarus 教程第 40 章实战', mtInformation, [mbOk], 0);
end;

procedure TMainForm.ActFindExecute(Sender: TObject);
begin
  FindDlg.Execute;                               // 非模态（25 章）：回调驱动
end;
