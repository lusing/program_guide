...
1. **只在 OnPaint 里画**（画了也会被下次重绘冲掉——17 章 TImage 的坑）。
...
`Picture.Bitmap.Canvas` 后 Invalidate 一次即可（17 章）。
...
`Canvas.TextOut(Rect.Left+2, Rect.Top+2, Items[Index])`。17 章的 TColorBox 就是
现成的自绘下拉。

## 7. 示例与验证

本章示例 `examples/22_paint`：离屏缓冲上预画线/矩形/椭圆/文字 + 鼠标画板 +
RenderTo 合成。selftest 六组像素断言全过（线段中点红、矩形内黄、椭圆心蓝、
文字起墨、笔迹命中、合成保真）。

```powershell
pwsh -File build.ps1 -Example 22_paint
```
...
---
上一章：[21 列表与树视图](21-lists.md) ｜ 下一章：[23 多线程与后台任务](23-threads.md) ｜ 返回：[README](../README.md)
