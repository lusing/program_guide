...
（11 章 INI/TextFile 同族坑的 GUI 版）。24 章的编码选择功能就是围绕它做的。

## 8. 示例与验证

本章示例 `examples/20_dialogs`：两个文件对话框（过滤器/选项全配置）、
FindDialog 回调、FontDialog 属性、ConfirmClose 纯逻辑函数、ModalResult 常量表。
selftest 全部走**无 UI 依赖路径**（属性 + 回调 + 纯函数）——弹窗类的 Execute
留给正常路径人工体验。

```powershell
pwsh -File build.ps1 -Example 20_dialogs
```
...
---
上一章：[19 菜单、工具栏与 Action](19-menus.md) ｜ 下一章：[21 列表与树视图](21-lists.md) ｜ 返回：[README](../README.md)
