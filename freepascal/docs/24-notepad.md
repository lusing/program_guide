...
前 23 章的知识点在这里全部上战场：**多标签文本编辑器**——动态标签页、文件打开/保存
（编码选择）、查找/替换、修改跟踪、状态栏统计、选项持久化。工程结构：

```text
examples/24_notepad_plus/
├── 24_notepad_plus.lpi    工程文件（3 个单元）
├── 24_notepad_plus.lpr    主程序：selftest 分支 + 主窗体启动
...
begin
  ...
  Result.Memo.Align := alClient;          // 编辑器吃满整页（18 章）
  ...
end;
...
```pascal
// 打开：显式 UTF-8（20 章坑：默认按系统 ANSI 读，中文必乱）
...
剥出来**，是这段代码可无头测试的全部原因（20 章 ModalResult 的实战收尾）。
...
```powershell
pwsh -File build.ps1 -Example 24_notepad_plus
# lazbuild → exe --selftest → selftest.log 四条判定（含 ==== 24 selftest OK ====）
```
