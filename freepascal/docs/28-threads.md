...
- 只读共享（快照）不需要锁——27 章画板"数据在主线程、线程只读"就是免锁设计。
...
本章示例 `examples/28_threads`：TWorker（求和 + 锁 + Synchronize 汇报 + 取消点）、
...
```powershell
pwsh -File build.ps1 -Example 28_threads
```
...
---
上一章：[27 绘图与自绘](27-canvas.md) ｜ 下一章：[29 应用与窗体生命周期](29-form-lifecycle.md) ｜ 返回：[README](../README.md)
