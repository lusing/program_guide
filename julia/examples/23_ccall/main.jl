# 23 C 互操作：ccall/@ccall、类型映射、Cstring/指针、@cfunction 回调、跨平台库名
# 运行：julia --startup-file=no main.jl

# ═══ 23.1 类型映射表：Julia 类型 ↔ C 类型（ABI 层面等价）
# C 类型     Julia 类型        说明
...
# ═══ 23.5 Windows API：stdcall 不用声明（x64 只有一种调用约定）
if Sys.iswindows()
...
# ═══ 23.6 @cfunction：把 Julia 函数变成 C 函数指针（回调）
