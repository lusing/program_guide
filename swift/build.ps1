...
$swiftCandidates = @(
    "G:\scoop\apps\swift\current\Toolchains\usr\bin\swiftc.exe",
    "G:\scoop\apps\swift\current\Toolchains\6.3.3+NoAsserts\usr\bin\swiftc.exe"
)

$swiftc = $swiftCandidates | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
if (-not $swiftc) {
    throw "未找到 swiftc.exe，请检查 Swift 安装路径。"
}
...
if ($All) {
    $files = Get-ChildItem -LiteralPath $examplesDir -Filter "*.swift" | Sort-Object Name
    foreach ($f in $files) {
...
Write-Host "  .\build.ps1 -File <name.swift>  编译单个示例"
