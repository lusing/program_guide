...
对应示例：`../examples/24_numbers.v`

### 24.1 nat 的成本模型：数学家的数 vs 工程师的数
...
实测对比（示例 24）：`Nat.pow 2 16`（65536 个 S）还能瞬间算完并打印；`Z.pow 2 64`、`Z.pow 2 100` 输出完整十进制也是瞬间。
...
| 性能关键的已验证代码 | Coq 里证明，Extraction 抽取成 OCaml 后跑（第 32 章） |

互通的桥：`Z.of_nat` / `Z.to_nat` / `N.of_nat` / `N.to_nat`。注意转换本身有成本（一元 ↔ 二进制是表示形状的整体改写），**在边界一次转换、内部统一数系**是工程习惯。

### 24.3 lia：算术证明的自动化

前 23 章证过 `n + 0 = n`、`plus_comm`——纯算术的体力活。标准库的 `lia`（linear integer arithmetic，来自 `Lia`）把这类活自动包了：
...
---
上一章：[23 · 插入排序与正确性证明](23-sorting.md) ｜ 下一章：[25 · 测试与断言风格](25-testing.md) ｜ 返回：[README](../README.md)
