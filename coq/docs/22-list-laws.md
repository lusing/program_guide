...
对应示例：`../examples/22_list_laws.v`

### 22.1 本章任务

列表是函数式编程的「数组」，它的定律就是日常重构的理论基础。六条（示例 22 全部证毕）：
...
1. **累加器命题没一般化**：`rev_acc [] xs` 直接归纳 IH 够不着，改成 `forall acc, rev_acc acc xs = rev xs ++ acc`（22.3 的完整案例）；
...
---
上一章：[21 · 表达式求值器：AST 入门](21-ast.md) ｜ 下一章：[23 · 插入排序与正确性证明](23-sorting.md) ｜ 返回：[README](../README.md)
