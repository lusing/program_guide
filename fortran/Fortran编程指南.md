...
一份面向「会写别的语言，但没写过 Fortran」的人的教程。全书 24 章，配 22 个可运行示例，每个示例都在 **LLVM flang 23** 和 **GNU Fortran 15** 两个编译器上实测通过。
...
**Free form vs fixed form**：`.f90` 后缀是自由格式（可以随意缩进，一行 132 列上限），`.f` 是固定格式（第 1–5 列是标号区，第 6 列是续行标志）。本仓库全部用 `.f90`。固定格式只在你需要维护 1980 年代的代码时才有意义。
