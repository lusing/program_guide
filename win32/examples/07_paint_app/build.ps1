...
& $clPath /nologo /EHsc /DUNICODE /D_UNICODE /I"$includeDir" /I"$windowsIncludeDir" /I"$windowsSharedDir" /I"$ucrtIncludeDir" /Fo"$buildDir\" /Fe"$buildDir\07_paint_app.exe" "$PSScriptRoot\main.cpp" /link /MACHINE:X64 /SUBSYSTEM:WINDOWS /LIBPATH:"$libDir" /LIBPATH:"$windowsLibDir" /LIBPATH:"$ucrtLibDir" user32.lib gdi32.lib shell32.lib kernel32.lib

if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host 'Build succeeded: ' "$buildDir\07_paint_app.exe"
