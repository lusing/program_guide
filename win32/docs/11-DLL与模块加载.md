...
适用场景：**插件系统**（扫描目录逐个加载）、**可选功能**（缺 DLL 只禁用该功能）、**延迟加载**（启动提速）、**绕过版本差异**（新 API 不存在时降级——第 5 章 DPI 的兼容写法就可以这样实现）。
...
导入表（依赖谁、要哪些函数）、导出表（给别人什么）就在可选头的 DataDirectory 里指向。理解 PE 的回报：读得懂 `dumpbin`/`Dependencies` 工具输出、理解为什么 DLL 能被内存映射加载（第 9 章的映射机制！）、理解签名与防篡改。深入读《Peering Inside the PE》或用工具对照真文件，一天即可入门。

## 11.7 资源段：exe 里还能住什么

`.rsrc` 节存放资源：图标、光标、对话框模板、字符串表、版本信息、自定义二进制。第 6 章的对话框模板就住这里。资源的身份 = 类型 + 名称 + 语言，代码侧统一用 `FindResource`/`LoadResource`/`LockResource` 三步取：
...
**下一章**：[第 12 章 现代 Win32 与学习路线](12-现代Win32与学习路线.md)——Win10/11 新增 API 清单、与上层框架的关系图谱、全书避坑总表。
