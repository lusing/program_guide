...
- [android](./android) — Android 应用开发教程（Kotlin，16 章分章文档 docs/：平台架构 / 工程工具链 / Kotlin 子集 / Activity 生命周期 / 传统 View 体系 / Intent / 列表 / 线程网络 / 存储 / 广播 Service 通知 / 权限硬件 / **Jetpack Compose 三部曲**（12–14）/ JNI NDK / 实战 MemoPad 便签应用——传统 View 与 Compose 双 UI 范式并存），三层编译验证：`examples/` 21 条 Kotlin API 示例（kotlinc + android.jar 静态编译）+ `compose_examples/` Gradle 工程（AGP 8.7.3 / Compose 1.7.8，compileDebugKotlin，含 23 条 Compose 示例）+ JNI（NDK 30 + CMake 交叉编译 libguide_native.so）；运行需真机不在验证范围；build.ps1 为 UTF-8 无 BOM 须 PowerShell 7，详见 [android/README.md](./android/README.md)
- [asm/intel](./asm/intel) — x86-64 汇编编程指南，三平台验证：Windows 用 NASM `-f win64` + MSVC link.exe（58 个），macOS 用 NASM `-f macho64` + clang/ld（56 个），Linux 用 NASM `-f elf64` + `gcc -no-pie`（56 个）。macOS 侧已在 macOS 13.1/14.8.9 两套 Intel 配置上各实测 56/56（本机复核修掉了 PIE 绝对地址重定位、构建脚本 BSD grep 判据、ld64 参数改名三个问题）
