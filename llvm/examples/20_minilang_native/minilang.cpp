// MiniLang v0.9 —— 第 20 章：编译到原生目标文件（.exe 的最后一公里）
//
// 新增：
...
int main(int argc, char **argv) {
  ...
  if (argc == 4 && std::string(argv[1]) == "--obj")
  ...
  if (argc == 3 && std::string(argv[1]) == "--jit0")
  ...
  std::cerr << "usage: minilang --ast <file.mini>\n"
               "       minilang --jit <file.mini>      (JIT，编译前 -O2)\n"
               "       minilang --jit0 <file.mini>     (JIT，不优化——对照)\n"
