; 第 10 章示例：同一段 IR，编译到不同目标
; 本机：llc demo.ll -filetype=obj -o demo.o && clang demo.o -o demo.exe
; 交叉：llc --mtriple=aarch64-linux-gnu demo.ll -o demo.aarch64.s（只看汇编）

@.intfmt = private constant [4 x i8] c"%d\0A\00"
@.ok = private constant [16 x i8] c"==== 10 ok ====\00"

declare i32 @printf(ptr, ...)
declare i32 @puts(ptr)

define i32 @square(i32 %x) {
entry:
  %r = mul i32 %x, %x
  ret i32 %r
}

; 两层循环：给指令调度/寄存器分配一点活干
define i32 @sum_squares(i32 %n) {
entry:
  %i.addr = alloca i32
  %acc.addr = alloca i32
  store i32 1, ptr %i.addr
  store i32 0, ptr %acc.addr
  br label %loop.cond

loop.cond:
  %i = load i32, ptr %i.addr
  %cond = icmp sle i32 %i, %n
  br i1 %cond, label %loop.body, label %loop.end

loop.body:
  %sq = call i32 @square(i32 %i)
  %acc = load i32, ptr %acc.addr
  %acc.next = add i32 %acc, %sq
  store i32 %acc.next, ptr %acc.addr
  %i.next = add i32 %i, 1
  store i32 %i.next, ptr %i.addr
  br label %loop.cond

loop.end:
  %r = load i32, ptr %acc.addr
  ret i32 %r
}

define i32 @main() {
entry:
  %a = call i32 @sum_squares(i32 10)
  %pf = call i32 (ptr, ...) @printf(ptr @.intfmt, i32 %a)   ; 385
  %pm = call i32 @puts(ptr @.ok)
  ret i32 0
}
