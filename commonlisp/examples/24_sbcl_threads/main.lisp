;;;; ============================================================
;; channel: sbcl   （SBCL 专属扩展：依赖 sb-thread / sb-alien / sb-profile）
;;;; examples/24_sbcl_threads/main.lisp — 多线程编程（sb-thread）
;;;; ============================================================
;;;;
;;;; 本例程演示：
;;;;   0. 线程安全的打印（自己写一个 SAY）
...
;;;; 运行方式：sbcl --script examples/24_sbcl_threads/main.lisp
;;;;   等价、且不依赖 shebang 的写法（验证脚本用的就是这个）：
;;;;   sbcl --noinform --non-interactive --no-userinit --load examples/24_sbcl_threads/main.lisp
;;;;
;;;; ------------------------------------------------------------
