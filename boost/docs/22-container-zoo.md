...
> 实测坑（跨平台/跨版本）：ICL 区间的**打印形式会随版本变**。同一个 `interval_map`，Boost 1.92（MSVC）打 `[0,4) → 1 / [10,14] → 1`，Boost 1.88（clang）打 `[0,5) → 1 / (9,14] → 1`——段数与值都一样，只有界符不同。断言要比**段数和值**，别比界符字符串。
