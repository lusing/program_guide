...
同种子的裸引擎序列一致? true
std/boost 同种子分布一致? true
不播种的两个引擎同序列? true（默认构造 = 固定种子，注意！）
自检通过
```

运行输出（`random.cpp`，macOS 侧；只有第 5 行不同）：
...
身高直方图（每 10cm 一档，取 160-180）：160:451 170:456 180:41 
同种子的裸引擎序列一致? true
std/boost 同种子分布一致? false（分布映射算法标准未规定，随 STL 变）
不播种的两个引擎同序列? true（默认构造 = 固定种子，注意！）
自检通过
```

> **"引擎确定"不等于"分布确定"**——这是换到 macOS 才暴露出来的一条：
> `mt19937` 的算法被标准写死，同种子同**裸序列**在任何实现上都一致（第 3 行两平台
> 都是 `true`）；但 `uniform_int_distribution` 把 32 位随机值映射成 `[1,6]` 的那一步，
> 标准只规定结果的分布、不规定映射算法，各家 STL 自己实现——MSVC 的 STL 与 Boost
> 恰好一致，libc++ 不一致（第 4 行：Windows `true` / macOS `false`）。
> 早先例程只打一行"同种子序列一致?"，等于把两者混为一谈了。

四个工程要点：

1. **同种子同序列**（上面第 3 行实测 `true`，**指引擎**）：`mt19937` 是数值定义确定的算法——这对**可复现测试**至关重要（本教程所有随机示例都固定种子，输出才能进文档）。
