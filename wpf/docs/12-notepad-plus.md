...
> 对应示例：`examples/10_notepad_plus`——把全书知识串成的一个完整应用。
...
| 菜单 + KeyBinding 快捷键 + 命令绑定 | 03、07 |
| 三行 Grid 骨架（菜单/编辑区/状态栏） | 04 |
| 菜单项 ItemsSource 绑定集合 | 05、06 |
| MVVM：ViewModelBase + RelayCommand + 命令表 | 07 |
| DataTrigger 驱动自动换行 | 08 |
| 异步文件读写 + 状态栏反馈 | 09 |
| 打开/保存对话框（委托注入 ViewModel） | 10 |
| 脏标记与退出确认（OnClosing） | 02、07 |
| 最近文件列表（JSON 持久化） | 06、07 |
| 非模态查找替换窗口 | 10 |
| 编码识别（BOM/UTF-8/GB18030） | 09（IO）+ 本章 |
...
├── ViewModels/
│   ├── ViewModelBase.cs       # 第 07 章的标准壳
│   ├── RelayCommand.cs        # 第 07 章的命令实现
...
这个划分本身就是实践建议：**ViewModel 里一行 `using System.Windows.Controls` 都没有**——它知道"有一个字符串叫 Content"，不知道有 TextBox。查找替换的字符串算法（FindNext/ReplaceAll）是纯函数式的，可以脱离界面单元测试；窗口只负责选区和提示。
...
ViewModel 不碰 UI，但确实需要"弹文件对话框"和"开查找窗口"。两种手法都在实战里：
...
这条边界线划清后，VM 在单元测试里给委托塞个假路径就能跑完整流程。

## 5. 编码识别：三层判断

`EncodingDetector` 是第 10 章"实战必须补的两件事"之一，逻辑三层递进：
...
- **GB18030 不是免费的**：.NET 默认只带 Unicode 系编码，代码页编码要在 `App` 构造函数里注册提供程序（第 02 章 OnStartup 的应用场景）：`Encoding.RegisterProvider(CodePagesEncodingProvider.Instance)`
...
`OpenAsync` 是第 09 章模式 + 第 06 章绑定的合流：
...
注意 `Content = content` 会触发 Content 的 setter 把 IsDirty 置 true（任何编辑都置脏），所以**随后显式清掉**——顺序不能反。整条链上没有一个控件被引用，全是属性。
...
XAML 侧的菜单条目动态生成（第 05 章 ItemsControl 机制的变体）：
...
`RelativeSource AncestorType=Window` 是条目模板里够到 ViewModel 的标准桥——条目的 DataContext 是字符串本身，命令在窗口的 DataContext（MainViewModel）上。
...
查找窗口用 `Show()`（非模态）：用户边看文档边替换；重复打开只 `Activate()` 置前（第 10 章的防重复套路）。
...
标题栏的 `*` 脏标记由 `Title` 计算属性 + `OnPropertyChanged(nameof(Title))` 联动（第 07 章 SetField 返回值的联动用法），用户不用猜有没有未保存的修改。

## 10. 可以继续做的事

把这个项目当练习基地，按难度递增：

1. 撤销/重做接入 TextBox 内建的 Undo（`Editor.Undo()`/`CanUndo`）并接到命令系统
2. 状态栏加"选区字符数"，用 IProgress 处理大文本的统计防抖
3. 多标签编辑（TabControl + 每标签一个 ViewModel）
4. 拖放打开文件（`AllowDrop` + `Drop` 事件）
5. 全局主题切换（App.Resources 的 DynamicResource，第 08 章）
6. 把查找替换升级为"正则模式"（`Regex.Matches` 进 FindNext）
7. 打包：`dotnet publish -r win-x64 --self-contained` 产出免安装 exe

---
上一章：[11 窗口与页面导航](11-navigation.md) ｜ 返回：[README](../README.md)
